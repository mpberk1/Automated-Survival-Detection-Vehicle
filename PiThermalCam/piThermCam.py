...
class pithermalcam:
    ...
    def _setup_therm_cam(self):
        ...
        try:
            ...
            logger.error(f"Error setting up thermal camera: {e}")
    ...
    def _process_raw_image(self):
        ...
        # Image processing
        # Can't apply colormap to integers
        if self._interpolation_index==5: # Pure scipy
            # Reshape image to matrix
            img = self._raw_image.reshape((24,32))
            # Scale up to the requested size with scipy
            img = ndimage.zoom(img, (self.image_height/24, self.image_width/32))
            self._image = img
    ...
    def _print_shortcuts_keys(self):
        ...
        print("The following keys are shortcuts for controlling the display:")
        print("Esc - Exit and Close.")
        print("s - Save a Snapshot of the Current Frame")
        print("c - Cycle through Color Maps")
        print("x - Cycle backwards through Color Maps")
        print("f - Toggle Filtering On/Off")
        print("t - Toggle Temperature Units")
        print("u - Change Interpolation Algorithm")
        print("i - Change Interpolation Algorithm (backwards)")
    ...
    def get_image_frame_for_gui(self):
        """Get a processed thermal image frame suitable for GUI display
        
        Returns:
            numpy.ndarray: Processed thermal image as a numpy array
        """
        try:
            # Update the raw image and process it
            self._pull_raw_image()
            self._process_raw_image()
            self._add_image_text()
            self._current_frame_processed = True
            
            # Return a copy of the image to avoid GUI manipulations affecting the original
            return self._image.copy()
        except Exception as e:
            logger.error(f"Error getting image frame for GUI: {e}")
            logger.error(traceback.format_exc())
